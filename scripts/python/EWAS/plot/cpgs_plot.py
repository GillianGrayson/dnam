...
for cpg_id, cpg in enumerate(cpgs):
    ...
    statistic, pvalue = mannwhitneyu(df.loc[df[status_col] == status_dict['Control'], cpg].values,
                                     df.loc[df[status_col] == status_dict['Case'], cpg].values)
    box = go.Figure()
    add_violin_trace(box, df.loc[df[status_col] == status_dict['Control'], cpg].values, get_status_names['Control'])
    add_violin_trace(box, df.loc[df[status_col] == status_dict['Case'], cpg].values, get_status_names['Case'])
    ...
    for name_cont, feat_cont in continuous_vars.items():
        ...
        for feat, groups in categorical_vars.items():
            ...
            if not os.path.exists(f"{path_save}/{name_cont}"):
                os.makedirs(f"{path_save}/{name_cont}")
            save_figure(fig, f"{path_save}/{name_cont}/{cpg_id}_{cpg}")
