...
pathlib.Path(f"{path_wd}/all_together/KW/fig").mkdir(parents=True, exist_ok=True)
pathlib.Path(f"{path_wd}/all_together/cpgs").mkdir(parents=True, exist_ok=True)

pheno_all = pd.read_pickle(f"{path_wd}/origin/pheno_all.pkl")
mvals_all = pd.read_csv(f"{path_wd}/all_together/mvals_combat.txt", delimiter="\t", index_col='row.names.tmpCombat.')
...
pheno_train_val.to_pickle(f"{path_wd}/all_together/pheno_train_val.pkl")
pheno_train_val.to_excel(f"{path_wd}/all_together/pheno_train_val.xlsx", index=True)
mvals_train_val.to_pickle(f"{path_wd}/all_together/mvals_train_val.pkl")

df_test = df_all.loc[df_all['Dataset'].isin(datasets_test), :]
pheno_test = df_test.loc[:, feats]
mvals_test = df_test.loc[:, cpgs]
pheno_test.to_pickle(f"{path_wd}/all_together/pheno_test.pkl")
pheno_test.to_excel(f"{path_wd}/all_together/pheno_test.xlsx", index=True)
mvals_test.to_pickle(f"{path_wd}/all_together/mvals_test.pkl")

cpgs_metrics_df = KW_Control(datasets_train_val, manifest, df_train_val, cpgs, f"{path_wd}/all_together/KW", "M value")
...
cpgs_metrics_df.to_excel(f"{path_wd}/all_together/cpgs/{cpgs_metrics_df.shape[0]}.xlsx", index=True)
