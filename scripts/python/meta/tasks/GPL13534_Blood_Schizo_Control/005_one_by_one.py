...
pathlib.Path(f"{path_wd}/one_by_one/KW/fig").mkdir(parents=True, exist_ok=True)
pathlib.Path(f"{path_wd}/one_by_one/cpgs").mkdir(parents=True, exist_ok=True)
...
pheno_train_val.to_pickle(f"{path_wd}/one_by_one/pheno_train_val.pkl")
pheno_train_val.to_excel(f"{path_wd}/one_by_one/pheno_train_val.xlsx", index=True)
mvals_train_val.to_pickle(f"{path_wd}/one_by_one/mvals_train_val.pkl")

df_test = df_all.loc[df_all['Dataset'].isin(datasets_test), :]
pheno_test = df_test.loc[:, feats]
mvals_test = df_test.loc[:, cpgs]
pheno_test.to_pickle(f"{path_wd}/one_by_one/pheno_test.pkl")
pheno_test.to_excel(f"{path_wd}/one_by_one/pheno_test.xlsx", index=True)
mvals_test.to_pickle(f"{path_wd}/one_by_one/mvals_test.pkl")

cpgs_metrics_df = KW_Control(datasets_train_val, manifest, df_train_val, cpgs, f"{path_wd}/one_by_one/KW", "M value")
...
cpgs_metrics_df.to_excel(f"{path_wd}/one_by_one/cpgs/{cpgs_metrics_df.shape[0]}.xlsx", index=True)
