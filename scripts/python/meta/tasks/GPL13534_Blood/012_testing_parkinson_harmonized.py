...
pathlib.Path(f"{path_wd}/harmonized").mkdir(parents=True, exist_ok=True)
...
for d_id, dataset in enumerate(datasets_trn_val):
    ...
    mvals = pd.read_csv(f"{path_wd}/harmonized/r/mvalsT_{dataset}_regRCPqn.txt", delimiter="\t", index_col='ID_REF')
    mvals = mvals.T
...
df_combo.to_pickle(f"{path_wd}/harmonized/data_GSE145361_GSE111629.pkl")
pheno_combo.to_excel(f"{path_wd}/harmonized/pheno_GSE145361_GSE111629.xlsx", index=True)
...
for dataset in dataset_statuses:
    ...
    df.to_pickle(f"{path_wd}/harmonized/data_{dataset}.pkl")
    pheno.to_excel(f"{path_wd}/harmonized/pheno_{dataset}.xlsx", index=True)
