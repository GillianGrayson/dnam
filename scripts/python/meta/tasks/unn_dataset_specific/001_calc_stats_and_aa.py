...
metrics = ['R2', 'R2_adj', 'mw_stat', 'mw_pval']
...
for f_id, f in enumerate(calc_features):
    ...
    scatter = go.Figure()
    add_scatter_trace(scatter, pheno_gse["Age"].values, pheno_gse[f].values, "GSE152026")
    ...
    res_dict['mw_stat'][f_id] = stat
    res_dict['mw_pval'][f_id] = pval

    vio = go.Figure()
    add_violin_trace(vio, pheno_gse[f"{f}Acc"].values, "GSE152026")
...
add_histogram_trace(fig, df_gse['Age'].values, f"GSE152026 ({df_gse.shape[0]})", 5.0)
add_layout(fig, "Age", "Count", "")
fig.update_layout(colorway = ['blue', 'red'], barmode = 'overlay')
save_figure(fig, f"{path_save}/histogram")
...
for cpg_id, cpg in tqdm(enumerate(cpgs), desc=f'Mann-Whitney', total=len(cpgs)):
...
#     add_box_trace(fig, df_gse[cpg].values, 'GSE152026')
...
#     add_violin_trace(fig, df_gse[cpg].values, 'GSE152026')
#     add_layout(fig, '', "Methylation Level", f"{cpg} ({manifest.loc[cpg, 'Gene']}): {row['mw_pval_fdr_bh']:0.4e}")
#     fig.update_layout({'colorway': ['blue', 'red']})
