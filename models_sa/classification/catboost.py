...
def train_catboost(config: DictConfig):
    ...
    metrics_classes_dict = get_metrics_dict(config.output_dim, object)
    metrics_summary = {
        'accuracy_macro': 'max',
    ...
    for epoch_id in metrics_train.iloc[:, 0]:
        log_dict = {
            'epoch': metrics_train.iat[epoch_id, 0],
            'train/loss': metrics_train.iat[epoch_id, 1],
            'val/loss': metrics_test.iat[epoch_id, 1]
        }
        wandb.log(log_dict)
